...
export default function EditSurveyPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<Survey>(`/surveys/${params.id}`, {
      ...
      .then((survey) => {
        ...
        setTargetRoles(survey.targetRoles);
        setIsActive(survey.isActive);
        setCanToggleStatus(survey.author?.id === currentUser?.id);
        setQuestions(
          (survey.questions || []).map((question: SurveyQuestion) => ({
            id: question.id,
            text: question.text,
            type: question.type,
            options: question.options.length > 0 ? question.options : ['Opcion 1'],
          })),
        );
      })
  ...
  function changeQuestionType(index: number, type: EditableQuestion['type']) {
    setQuestions((prev) =>
      prev.map((question, i) => {
  ...
  function updateQuestion(index: number, partial: Partial<EditableQuestion>) {
    setQuestions((prev) => prev.map((question, i) => (i === index ? { ...question, ...partial } : question)));
  }
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      await apiFetch(`/surveys/${params.id}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          title,
          ...
          questions: questions.map((question, index) => ({
            ...
            text: question.text,
            type: question.type,
            options: question.type === 'MULTIPLE_CHOICE' ? question.options.map((item) => item.trim()).filter(Boolean) : [],
            order: index + 1,
          })),
        }),
      });
      setStatus('Encuesta actualizada correctamente.');
    } catch (err) {
      setStatus(err instanceof Error ? err.message : 'No se pudo actualizar la encuesta');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen px-4 py-6 sm:px-6 sm:py-8 lg:px-10">
      <div className="glass-card soft-enter mx-auto max-w-4xl rounded-3xl border border-[var(--border)] p-6 shadow-sm sm:p-8">
        <h1 className="text-2xl font-semibold text-[var(--foreground)] sm:text-3xl">Editar encuesta</h1>
        <p className="mt-2 text-[var(--muted)]">Actualiza preguntas, estado y público objetivo.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-medium ${isActive ? 'bg-cyan-100 text-cyan-800' : 'bg-slate-200 text-slate-700'}`}
            >
              {isActive ? 'Activa' : 'Inactiva'}
            </button>
            {!canToggleStatus && (
              <p className="text-xs text-[var(--muted)]">Solo el dueño de la encuesta puede cambiar el estado.</p>
            )}
          </div>
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addQuestion} className="rounded-xl border border-[var(--primary)] px-4 py-2 text-sm font-medium text-[var(--primary)] hover:bg-[var(--primary-soft)]">
                Agregar pregunta
              </button>
            </div>

            {questions.map((question, index) => (
              <div key={`${question.id || 'new'}-${index}`} className="rounded-2xl border border-[var(--border)] bg-white/80 p-5">
                <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    <select
                      className="w-full rounded-xl border border-[var(--border)] px-4 py-3"
                      value={question.type}
                      onChange={(e) => changeQuestionType(index, e.target.value as EditableQuestion['type'])}
                    >
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {question.options.map((option, optionIndex) => (
                          <div key={`${question.id || 'new'}-${optionIndex}`} className="flex items-center gap-3">
